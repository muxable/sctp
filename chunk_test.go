...
func TestSelectAckChunk(t *testing.T) {
	...
	rawPkt := []byte{
		...
		0x13, 0x88, 0x13, 0x88, //source val num, destination val num
		0xc2, 0x98, 0x98, 0x0f, //verification tag
		0x44, 0xef, 0x5e, 0x54, //checksum

		0x03, 0x00, 0x00, 0x18, //type, chunk flags, chunk length

		0x87, 0x73, 0xbd, 0xa4, //Cumulative TSN Ack
		0x00, 0x01, 0xfe, 0x74, //a_rwnd
		0x00, 0x01, 0x00, 0x01, //Number of Gap ACK Blocks, Number of Duplicate TSNs
		0x00, 0x02, 0x00, 0x02, //Gap ack block start, gap ack block end
		0x00, 0x02, 0x00, 0x02, //Dup TSN
	}
	err := pkt.unmarshal(rawPkt)
	if err != nil {
		t.Errorf("Unmarshal failed, has chunk: %v", err)
...
func TestNRSackChunk(t *testing.T) {
	pkt := &packet{}
	...
	rawPkt := []byte{
		...
		0x13, 0x88, 0x13, 0x88, //source val num, destination val num
		0xc2, 0x98, 0x98, 0x0f, //verification tag
		0x64, 0x8e, 0x7e, 0xe6, //checksum

		0x10, 0x00, 0x00, 0x20, //type, chunk flags, chunk length

		0x87, 0x73, 0xbd, 0xa4, //Cumulative TSN Ack
		0x00, 0x01, 0xfe, 0x74, //a_rwnd
		0x00, 0x01, 0x00, 0x01, //Number of RGap ACK Blocks, Number of NR Gap Ack blocks
		0x00, 0x01, 0x00, 0x00, //Number of duplicate TSNs, Reserved
		0x00, 0x02, 0x00, 0x02, //RGapackblock start, end
		0x00, 0x02, 0x00, 0x02, //NRGapackblock start, end
		0x00, 0x02, 0x00, 0x02, //Duplicate TSN
	}
	...
	_, ok := pkt.chunks[0].(*chunkNRSack)
	if !ok {
		t.Error("Failed to cast Chunk -> NRSack")
	}
}
